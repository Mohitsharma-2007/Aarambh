...
logger = get_logger('aarambh.api.simulation')


# Interview prompt optimization prefix
# Adding this prefix can prevent Agent from calling tools and directly reply with text
...
def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize Interview prompts by adding prefix to avoid Agent calling tools
...
def get_graph_entities(graph_id: str):
    ...
    Only returns nodes with predefined entity types (Labels not just Entity)
...
def create_simulation():
    ...
    è¯·æ±‚ï¼ˆJSONï¼‰ï¼š
        {
            ...
            "graph_id": "aarambh_xxxx",    // å¯é€‰ï¼Œå¦‚ä¸æä¾›åˆ™ä»ŽprojectèŽ·å–
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "é¡¹ç›®å°šæœªæž„å»ºå›¾è°±ï¼Œè¯·å…ˆè°ƒç”¨ /api/graph/build"
            }), 400
        ...
        # Update project with simulation_id and status
        project.simulation_id = state.simulation_id
        project.simulation_status = state.status
        project.status = ProjectStatus.SIMULATION_CREATED
        ProjectManager.update_project(project)
...
def prepare_simulation():
    ...
    try:
        ...
        if not state:
            return jsonify({
                "success": False,
                "error": f"æ¨¡æ‹Ÿä¸å­˜åœ¨: {simulation_id}"
            }), 404
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "å·²æœ‰å®Œæˆçš„å‡†å¤‡å·¥ä½œï¼Œæ— éœ€é‡å¤ç”Ÿæˆ",
        ...
        if not project:
            return jsonify({
                "success": False,
                "error": f"é¡¹ç›®ä¸å­˜åœ¨: {state.project_id}"
            }), 404
        ...
        if not simulation_requirement:
            return jsonify({
                "success": False,
                "error": "é¡¹ç›®ç¼ºå°‘æ¨¡æ‹Ÿéœ€æ±‚æè¿° (simulation_requirement)"
            }), 400
        ...
        def run_prepare():
            try:
                ...
                def progress_callback(stage, progress, message, **kwargs):
                    ...
                    # æž„å»ºè¯¦ç»†è¿›åº¦ä¿¡æ¯
                    stage_names = {
                        "reading": "è¯»å–å›¾è°±å®žä½“",
                        "generating_profiles": "ç”ŸæˆAgentäººè®¾",
                        "generating_config": "ç”Ÿæˆæ¨¡æ‹Ÿé…ç½®",
                        "copying_scripts": "å‡†å¤‡æ¨¡æ‹Ÿè„šæœ¬"
                    }
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "preparing",
                "message": "å‡†å¤‡ä»»åŠ¡å·²å¯åŠ¨ï¼Œè¯·é€šè¿‡ /api/simulation/prepare/status æŸ¥è¯¢è¿›åº¦",
            ...
            if is_prepared:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "progress": 100,
                        "message": "å·²æœ‰å®Œæˆçš„å‡†å¤‡å·¥ä½œ",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
            ...
            if simulation_id:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "not_started",
                        "progress": 0,
                        "message": "å°šæœªå¼€å§‹å‡†å¤‡ï¼Œè¯·è°ƒç”¨ /api/simulation/prepare å¼€å§‹",
                        "already_prepared": False
                    }
                })
            return jsonify({
                "success": False,
                "error": "è¯·æä¾› task_id æˆ– simulation_id"
            }), 400
                ...
                if is_prepared:
                    return jsonify({
                        ...
                        "data": {
                            ...
                            "status": "ready",
                            "progress": 100,
                            "message": "ä»»åŠ¡å·²å®Œæˆï¼ˆå‡†å¤‡å·¥ä½œå·²å­˜åœ¨ï¼‰",
                            "already_prepared": True,
                            "prepare_info": prepare_info
                        }
                    })
...
def get_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not config:
            return jsonify({
                "success": False,
                "error": f"æ¨¡æ‹Ÿé…ç½®ä¸å­˜åœ¨ï¼Œè¯·å…ˆè°ƒç”¨ /prepare æŽ¥å£"
            }), 404
...
def download_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not os.path.exists(config_path):
            return jsonify({
                "success": False,
                "error": "é…ç½®æ–‡ä»¶ä¸å­˜åœ¨ï¼Œè¯·å…ˆè°ƒç”¨ /prepare æŽ¥å£"
            }), 404
...
def download_simulation_script(script_name: str):
    ...
    try:
        ...
        if script_name not in allowed_scripts:
            return jsonify({
                "success": False,
                "error": f"æœªçŸ¥è„šæœ¬: {script_name}ï¼Œå¯é€‰: {allowed_scripts}"
            }), 400
        ...
        if not os.path.exists(script_path):
            return jsonify({
                "success": False,
                "error": f"è„šæœ¬æ–‡ä»¶ä¸å­˜åœ¨: {script_name}"
            }), 404
...
def generate_profiles():
    ...
    try:
        ...
        if filtered.filtered_count == 0:
            return jsonify({
                "success": False,
                "error": "æ²¡æœ‰æ‰¾åˆ°ç¬¦åˆæ¡ä»¶çš„å®žä½“"
            }), 400
    ...
    try:
        ...
        if max_rounds is not None:
            try:
                ...
                if max_rounds <= 0:
                    return jsonify({
                        "success": False,
                        "error": "max_rounds å¿…é¡»æ˜¯æ­£æ•´æ•°"
                    }), 400
            except (ValueError, TypeError):
                return jsonify({
                    "success": False,
                    "error": "max_rounds å¿…é¡»æ˜¯æœ‰æ•ˆçš„æ•´æ•°"
                }), 400

        if platform not in ['twitter', 'reddit', 'parallel']:
            return jsonify({
                "success": False,
                "error": f"æ— æ•ˆçš„å¹³å°ç±»åž‹: {platform}ï¼Œå¯é€‰: twitter/reddit/parallel"
            }), 400
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": f"æ¨¡æ‹Ÿæ­£åœ¨è¿è¡Œä¸­ï¼Œè¯·å…ˆè°ƒç”¨ /stop æŽ¥å£åœæ­¢ï¼Œæˆ–ä½¿ç”¨ force=true å¼ºåˆ¶é‡æ–°å¼€å§‹"
                            }), 400
                ...
                return jsonify({
                    "success": False,
                    "error": f"æ¨¡æ‹Ÿæœªå‡†å¤‡å¥½ï¼Œå½“å‰çŠ¶æ€: {state.status.value}ï¼Œè¯·å…ˆè°ƒç”¨ /prepare æŽ¥å£"
                }), 400
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "å¯ç”¨å›¾è°±è®°å¿†æ›´æ–°éœ€è¦æœ‰æ•ˆçš„ graph_idï¼Œè¯·ç¡®ä¿é¡¹ç›®å·²æž„å»ºå›¾è°±"
                }), 400
        ...
        if not os.path.exists(db_path):
            return jsonify({
                ...
                "data": {
                    ...
                    "count": 0,
                    "posts": [],
                    "message": "æ•°æ®åº“ä¸å­˜åœ¨ï¼Œæ¨¡æ‹Ÿå¯èƒ½å°šæœªè¿è¡Œ"
                }
            })
...
def interview_agent():
    ...
    try:
        ...
        if agent_id is None:
            return jsonify({
                "success": False,
                "error": "è¯·æä¾› agent_id"
            }), 400
        
        if not prompt:
            return jsonify({
                "success": False,
                "error": "è¯·æä¾› promptï¼ˆé‡‡è®¿é—®é¢˜ï¼‰"
            }), 400
        ...
        if platform and platform not in ("twitter", "reddit"):
            return jsonify({
                "success": False,
                "error": "platform å‚æ•°åªèƒ½æ˜¯ 'twitter' æˆ– 'reddit'"
            }), 400
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "æ¨¡æ‹ŸçŽ¯å¢ƒæœªè¿è¡Œæˆ–å·²å…³é—­ã€‚è¯·ç¡®ä¿æ¨¡æ‹Ÿå·²å®Œæˆå¹¶è¿›å…¥ç­‰å¾…å‘½ä»¤æ¨¡å¼ã€‚"
            }), 400
        ...
        return jsonify({
            "success": False,
            "error": f"ç­‰å¾…Interviewå“åº”è¶…æ—¶: {str(e)}"
        }), 504
...
def interview_agents_batch():
    ...
    try:
        ...
        if not interviews or not isinstance(interviews, list):
            return jsonify({
                "success": False,
                "error": "è¯·æä¾› interviewsï¼ˆé‡‡è®¿åˆ—è¡¨ï¼‰"
            }), 400
        ...
        for i, interview in enumerate(interviews):
            if 'agent_id' not in interview:
                return jsonify({
                    "success": False,
                    "error": f"é‡‡è®¿åˆ—è¡¨ç¬¬{i+1}é¡¹ç¼ºå°‘ agent_id"
                }), 400
            if 'prompt' not in interview:
                return jsonify({
                    "success": False,
                    "error": f"é‡‡è®¿åˆ—è¡¨ç¬¬{i+1}é¡¹ç¼ºå°‘ prompt"
                }), 400
            ...
            if item_platform and item_platform not in ("twitter", "reddit"):
                return jsonify({
                    "success": False,
                    "error": f"é‡‡è®¿åˆ—è¡¨ç¬¬{i+1}é¡¹çš„platformåªèƒ½æ˜¯ 'twitter' æˆ– 'reddit'"
                }), 400
        ...
        return jsonify({
            "success": False,
            "error": f"ç­‰å¾…æ‰¹é‡Interviewå“åº”è¶…æ—¶: {str(e)}"
        }), 504
...
def interview_all_agents():
    ...
    try:
        ...
        return jsonify({
            "success": False,
            "error": f"ç­‰å¾…å…¨å±€Interviewå“åº”è¶…æ—¶: {str(e)}"
        }), 504
...
def get_env_status():
    ...
    try:
        ...
        if env_alive:
            ...
            message = "çŽ¯å¢ƒæœªè¿è¡Œæˆ–å·²å…³é—­"
