...
def get_project_state(project_id: str):
    ...
    # Determine current step based on project status and related entities
    current_step = 1  # Default: Graph Construction
    redirect_url = None

    if project.report_id and project.report_status == 'completed':
        current_step = 4  # Report completed
        redirect_url = f"/ontology/report/{project.report_id}"
    elif project.report_id and project.report_status == 'generating':
        current_step = 4  # Report generating
        redirect_url = f"/ontology/report/{project.report_id}?projectId={project_id}&graphId={project.graph_id or ''}"
    elif project.simulation_status == 'completed':
        current_step = 3  # Simulation completed, should create report
        redirect_url = f"/ontology/simulation/{project.simulation_id}?projectId={project_id}&graphId={project.graph_id or ''}"
    elif project.simulation_status in ['ready', 'running']:
        current_step = 3  # Simulation ready/running
        redirect_url = f"/ontology/simulation/{project.simulation_id}?projectId={project_id}&graphId={project.graph_id or ''}"
    elif project.simulation_id:
        current_step = 2  # Environment setup created
        redirect_url = f"/ontology/simulation/{project.simulation_id}?projectId={project_id}&graphId={project.graph_id or ''}"
    elif project.status == 'graph_completed':
        current_step = 2  # Graph done, need env setup
        redirect_url = None  # Stay on env setup
    elif project.status == 'graph_building':
        current_step = 1  # Graph building
        redirect_url = None  # Stay on graph construction
    elif project.status in ['ontology_generated', 'created']:
        current_step = 1  # Need to build graph
        redirect_url = None
...
def build_graph():
    """
    Interface 2: Build knowledge graph based on project_id
