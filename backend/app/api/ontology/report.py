"""
Report API Routes
Provides simulation report generation, retrieval, conversation interfaces
"""
...
logger = get_logger('aarambh.api.report')
...
def generate_report():
    ...
    try:
        ...
        # Update project with report_id and status
        project.report_id = report_id
        project.report_status = 'generating'
        project.status = ProjectStatus.REPORT_GENERATING
        ...
        def run_generate():
            try:
                ...
                if report.status == ReportStatus.COMPLETED:
                    # Update project report status to completed
                    project.report_status = 'completed'
                    project.status = ProjectStatus.REPORT_COMPLETED
                    ProjectManager.update_project(project)
                    ...
                    # Update project report status to failed
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "generating",
                "message": "æŠ¥å‘Šç”Ÿæˆä»»åŠ¡å·²å¯åŠ¨ï¼Œè¯·é€šè¿‡ /api/report/generate/status æŸ¥è¯¢è¿›åº¦",
...
def get_report(report_id: str):
    ...
    try:
        ...
        if not report:
            return jsonify({
                "success": False,
                "error": f"æŠ¥å‘Šä¸å­˜åœ¨: {report_id}"
            }), 404
...
def get_report_by_simulation(simulation_id: str):
    ...
    try:
        ...
        if not report:
            return jsonify({
                "success": False,
                "error": f"è¯¥æ¨¡æ‹Ÿæš‚æ— æŠ¥å‘Š: {simulation_id}",
                "has_report": False
            }), 404
...
def delete_report(report_id: str):
    ...
    try:
        ...
        return jsonify({
            "success": True,
            "message": f"æŠ¥å‘Šå·²åˆ é™¤: {report_id}"
        })
...
def get_report_progress(report_id: str):
    ...
    try:
        ...
        if not progress:
            return jsonify({
                "success": False,
                "error": f"æŠ¥å‘Šä¸å­˜åœ¨æˆ–è¿›åº¦ä¿¡æ¯ä¸å¯ç”¨: {report_id}"
            }), 404
...
def search_graph_tool():
    ...
    try:
        ...
        if not graph_id or not query:
            return jsonify({
                "success": False,
                "error": "è¯·æä¾› graph_id å’Œ query"
            }), 400
