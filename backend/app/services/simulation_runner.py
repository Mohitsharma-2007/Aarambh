...
logger = get_logger('aarambh.simulation_runner')
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        # å¯åŠ¨æ¨¡æ‹Ÿè¿›ç¨‹
        try:
            # æž„å»ºè¿è¡Œå‘½ä»¤ï¼Œä½¿ç”¨venv311çš„Pythonè§£é‡Šå™¨
            # èŽ·å–venv311çš„Pythonè·¯å¾„ï¼ˆä¸Žå½“å‰è¿è¡ŒçŽ¯å¢ƒä¸€è‡´ï¼‰
            venv_python = os.path.join(
                os.path.dirname(os.path.dirname(os.path.dirname(__file__))),
                'venv311', 'Scripts', 'python.exe'
            )
            # å¦‚æžœvenv311ä¸å­˜åœ¨ï¼Œå›žé€€åˆ°sys.executable
            if not os.path.exists(venv_python):
                venv_python = sys.executable
            
            cmd = [
                venv_python,  # ä½¿ç”¨venv311çš„Pythonè§£é‡Šå™¨
                script_path,
            ...
            # ä½¿ç”¨ start_new_session=True åˆ›å»ºæ–°çš„è¿›ç¨‹ç»„ï¼Œç¡®ä¿å¯ä»¥é€šè¿‡ os.killpg ç»ˆæ­¢æ‰€æœ‰å­è¿›ç¨‹
            process = subprocess.Popen(
                ...
                start_new_session=True,  # åˆ›å»ºæ–°è¿›ç¨‹ç»„ï¼Œç¡®ä¿æœåŠ¡å™¨å…³é—­æ—¶èƒ½ç»ˆæ­¢æ‰€æœ‰ç›¸å…³è¿›ç¨‹
            )
