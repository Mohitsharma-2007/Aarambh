...
Interface: Analyzes text content and generates entity and relationship type
definitions suitable for social media discourse simulation.
"""
...
ONTOLOGY_SYSTEM_PROMPT = """You are a professional knowledge graph ontology design expert. Your task is to analyze the given text content and simulation requirements, designing entity types and relationship types suitable for **social media discourse simulation**.

**IMPORTANT: You must output valid JSON data only. Do not output any other content.**

## Core Task Background

We are building a **social media discourse simulation system**. In this system:
- Each entity is an "account" or "actor" that can voice opinions, interact, and spread information on social media
- Entities interact with each other — reposting, commenting, replying
- We need to simulate each party's response in a discourse event and information propagation paths
...
- Viewpoints/attitudes (e.g., "supporters", "opposition")
...
A. **Fallback types (required, placed as the last 2 in the list)**:
   - `Person`: Fallback type for any individual person. When a person doesn't fit other more specific person types, they are classified here.
   - `Organization`: Fallback type for any organization. When an organization doesn't fit other more specific organization types, it's classified here.

B. **Specific types (8, designed based on text content)**:
   - Design more specific types based on the main roles that appear in the text
...
- Various types of people will appear in text, such as "school teacher", "passerby", "anonymous netizen"
...
- Person: Any individual (used when none of the above specific types apply)
...
- Organization: Any organization (used when none of the above specific types apply)

## Relationship Type Reference

- WORKS_FOR: Employment relationship
- STUDIES_AT: Enrollment relationship
- AFFILIATED_WITH: Affiliation
- REPRESENTS: Representation
- REGULATES: Regulation
- REPORTS_ON: Coverage/reporting
- COMMENTS_ON: Commentary
...
class OntologyGenerator:
    ...
    # Maximum text length for LLM input (50,000 chars)
    ...
    def _build_user_message(
        ...
        # Merge document texts
        combined_text = "\n\n---\n\n".join(document_texts)
        original_length = len(combined_text)
        
        # Truncate if text exceeds 50,000 chars (only affects LLM input, not graph construction)
...
"""
        ...
        message += """
Based on the above content, design entity types and relationship types suitable for social media discourse simulation.
...
4. All entity types must be real-world actors that can voice opinions, not abstract concepts
...
"""
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Final safety check — ensure limits are not exceeded (defensive programming)
    ...
    def generate_python_code(self, ontology: Dict[str, Any]) -> str:
        ...
        code_lines = [
            ...
            'Auto-generated by AARAMBH for social discourse simulation',
            '"""',
            '',
