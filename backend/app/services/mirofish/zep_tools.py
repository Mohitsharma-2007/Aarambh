...
1. InsightForge (Deep Insight Search) - Most powerful hybrid search, automatically generates sub-questions and multi-dimensional search
...
class InsightForgeResult:
    ...
    Contains search results from multiple sub-queries and comprehensive analysis
    """
    query: str
    simulation_requirement: str
    sub_queries: List[str]
    
    # Search results across dimensions
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        ...
        if self.active_facts:
            text_parts.append(f"\n### [Currently Active Facts] (Simulation result text)")
...
class AgentInterview:
    """Interview result from a single Agent"""
    agent_name: str
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                # Filter out garbage content with question numbers (questions 1-9)
                skip = False
                for d in '123456789':
                ...
                if skip:
                    continue
                # Truncate long content (truncate at period, not hard truncation)
...
class ZepToolsService:
    ...
    def _local_search(
        ...
        Get all edges/nodes, then match locally with keywords
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        """
        Get all edges in the graph (paginated fetch, includes time info)
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        By getting all edges in the graph and filtering for the specified node
    ...
    def get_entity_summary(
        ...
        """
        Get relationship summary for the specified entity
    ...
    def insight_forge(
        ...
        The most powerful hybrid retrieval function, auto-decomposes queries for multi-dimensional search:
        ...
        for uuid in list(entity_uuids):  # Process all entities, don't truncate
            if not uuid:
                continue
            try:
                ...
                if node:
                    ...
                    # Get all facts related to this entity (don't truncate)
                    related_facts = [
                    ...
                    entity_insights.append({
                        ...
                        "type": entity_type,
                        "summary": node.summary,
                        "related_facts": related_facts  # Full output, don't truncate
                    })
            except Exception as e:
        ...
        relationship_chains = []
        for edge_data in all_edges:  # Process all edges, don't truncate
            if isinstance(edge_data, dict):
                source_uuid = edge_data.get('source_node_uuid', '')
                target_uuid = edge_data.get('target_node_uuid', '')
...
Return the sub-question list in JSON format."""
    ...
    def panorama_search(
        ...
        3. Categorize current active vs historical information
        
        This tool is suitable for understanding the full picture and tracking event evolution.
    ...
    def interview_agents(
        ...
        4. Calls /api/simulation/interview/batch for real interviews (dual platform simultaneous)
        ...
        - Collecting multi-party opinions and viewpoints
        - Getting real Agent responses (not LLM-simulated)
