...
def get_project_state(project_id: str):
    """
    Get project state (aliased for frontend compatibility)
    """
    project = ProjectManager.get_project(project_id)
...
def build_graph():
    ...
    try:
        ...
        # If project failed previously, allow rebuild even without 'force'
        if project.status == ProjectStatus.FAILED and not force:
            logger.info(f"Retrying failed project build: {project_id}")
            project.error = None
            project.status = ProjectStatus.ONTOLOGY_GENERATED
