...
class GraphBuilderService:
    ...
    def create_graph(self, name: str) -> str:
        ...
        graph_id = f"aarambh_{uuid.uuid4().hex[:16]}"
        
        self.client.graph.create(
            graph_id=graph_id,
            name=name,
            description="aarambh Social Simulation Graph"
        )
        
        return graph_id
    
    def set_ontology(self, graph_id: str, ontology: Any):
        """è®¾ç½®å›¾è°±æœ¬ä½“ï¼ˆå…¬å¼€æ–¹æ³•ï¼‰"""
        import warnings
        import re
        import json
        
        # Defensive check: if ontology is a string, parse it as JSON
        if isinstance(ontology, str):
            try:
                ontology = json.loads(ontology)
            except Exception as e:
                raise ValueError(f"Ontology is a string but failed to parse as JSON: {str(e)}")
        
        if not isinstance(ontology, dict):
            raise ValueError(f"Ontology must be a dictionary or a JSON string, got {type(ontology)}")
        ...
        for entity_def in ontology.get("entity_types", []):
            ...
            # Enforce PascalCase for Zep compatibility
        ...
        for edge_def in ontology.get("edge_types", []):
            ...
            # åŠ¨æ€ åˆ›å»ºç±» (Edge classes also in PascalCase)
            ...
            for st in edge_def.get("source_targets", []):
                # Defensive: handle string items (e.g. just entity name) or dict items
                if isinstance(st, dict):
                    src = to_pascal_case(st.get("source", "Entity"))
                    tgt = to_pascal_case(st.get("target", "Entity"))
                elif isinstance(st, str):
                    logger.warning(f"source_target is a string '{st}', wrapping as source=target='{st}'")
                    src = to_pascal_case(st)
                    tgt = to_pascal_case(st)
                else:
                    logger.warning(f"Skipping invalid source_target: {st}")
                    continue
    ...
    def add_text_batches(
        ...
        for i in range(0, total_chunks, batch_size):
            ...
            if progress_callback:
                ...
                progress_callback(
                    f"å‘é€ç¬¬ {batch_num}/{total_batches} æ‰¹æ•°æ® ({len(batch_chunks)} å—)...",
                    progress
                )
            ...
            try:
                ...
                if progress_callback:
                    progress_callback(f"æ‰¹æ¬¡ {batch_num} å‘é€å¤±è´¥: {str(e)}", 0)
                raise
    ...
    def _wait_for_episodes(
        ...
        if not episode_uuids:
            if progress_callback:
                progress_callback("æ— éœ€ç­‰å¾…ï¼ˆæ²¡æœ‰ episodeï¼‰", 1.0)
            return
        ...
        if progress_callback:
            progress_callback(f"å¼€å§‹ç­‰å¾… {total_episodes} ä¸ªæ–‡æœ¬å—å¤„ç†...", 0)
        
        while pending_episodes:
            if time.time() - start_time > timeout:
                if progress_callback:
                    progress_callback(
                        f"éƒ¨åˆ†æ–‡æœ¬å—è¶…æ—¶ï¼Œå·²å®Œæˆ {completed_count}/{total_episodes}",
            ...
            if progress_callback:
                progress_callback(
                    f"Zepå¤„ç†ä¸­... {completed_count}/{total_episodes} å®Œæˆ, {len(pending_episodes)} å¾…å¤„ç† ({elapsed}ç§’)",
        ...
        if progress_callback:
            progress_callback(f"å¤„ç†å®Œæˆ: {completed_count}/{total_episodes}", 1.0)
    ...
    def get_graph_data(self, graph_id: str) -> Dict[str, Any]:
        ...
        for edge in edges:
            ...
            edges_data.append({
                ...
                "source_node_name": node_map.get(source_uuid, "") if source_uuid else "",
                "target_node_name": node_map.get(target_uuid, "") if target_uuid else "",
