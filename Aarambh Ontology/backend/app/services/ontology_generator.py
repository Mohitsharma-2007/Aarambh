...
```json
{
    ...
    "analysis_summary": "å¯¹æ–‡æœ¬å†…å®¹çš„ç®€è¦åˆ†æžè¯´æ˜Žï¼ˆä¸­æ–‡ï¼‰"
}
```
...
"""
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(result, dict):
            logger.error(f"Ontology result is not a dictionary: {type(result)}")
            return {"entity_types": [], "edge_types": [], "analysis_summary": "Failed to parse LLM response as a dictionary."}
        ...
        for entity in result["entity_types"]:
            ...
            if "name" not in entity:
                logger.warning(f"Skipping entity without 'name' key: {entity}")
                continue
        ...
        for edge in result["edge_types"]:
            if not isinstance(edge, dict):
                logger.warning(f"Skipping non-dict edge: {edge}")
                continue
                
            if "name" not in edge:
                logger.warning(f"Skipping edge without 'name' key: {edge}")
                continue
                
            if "source_targets" not in edge or not isinstance(edge["source_targets"], list):
                edge["source_targets"] = []
    ...
    def generate_python_code(self, ontology: Dict[str, Any]) -> str:
        ...
        code_lines = [
            ...
            'ç”±aarambhè‡ªåŠ¨ç”Ÿæˆï¼Œç”¨äºŽç¤¾ä¼šèˆ†è®ºæ¨¡æ‹Ÿ',
            '"""',
            '',
