...
logger = get_logger('aarambh.simulation_config')
...
class SimulationConfigGenerator:
    ...
    def generate_config(
        ...
        report_progress(1, "ç”Ÿæˆæ—¶é—´é…ç½®...")
        num_entities = len(entities)
        time_config_result = self._generate_time_config(context, num_entities)
        time_config = self._parse_time_config(time_config_result, num_entities)
        reasoning_parts.append(f"æ—¶é—´é…ç½®: {time_config_result.get('reasoning', 'æˆåŠŸ')}")
        
        # ========== æ­¥éª¤2: ç”Ÿæˆäº‹ä»¶é…ç½® ==========
        report_progress(2, "ç”Ÿæˆäº‹ä»¶é…ç½®å’Œçƒ­ç‚¹è¯é¢˜...")
        event_config_result = self._generate_event_config(context, simulation_requirement, entities)
        event_config = self._parse_event_config(event_config_result)
        reasoning_parts.append(f"äº‹ä»¶é…ç½®: {event_config_result.get('reasoning', 'æˆåŠŸ')}")
        ...
        for batch_idx in range(num_batches):
            ...
            report_progress(
                3 + batch_idx,
                f"ç”ŸæˆAgenté…ç½® ({start_idx + 1}-{end_idx}/{len(entities)})..."
            )
        ...
        reasoning_parts.append(f"Agenté…ç½®: æˆåŠŸç”Ÿæˆ {len(all_agent_configs)} ä¸ª")
        
        # ========== ä¸ºåˆå§‹å¸–å­åˆ†é…å‘å¸ƒè€… Agent ==========
        logger.info("ä¸ºåˆå§‹å¸–å­åˆ†é…åˆé€‚çš„å‘å¸ƒè€… Agent...")
        event_config = self._assign_initial_post_agents(event_config, all_agent_configs)
        assigned_count = len([p for p in event_config.initial_posts if p.get("poster_agent_id") is not None])
        reasoning_parts.append(f"åˆå§‹å¸–å­åˆ†é…: {assigned_count} ä¸ªå¸–å­å·²åˆ†é…å‘å¸ƒè€…")
        
        # ========== æœ€åŽä¸€æ­¥: ç”Ÿæˆå¹³å°é…ç½® ==========
        report_progress(total_steps, "ç”Ÿæˆå¹³å°é…ç½®...")
        twitter_config = None
        reddit_config = None
...
- ç”¨æˆ·ç¾¤ä½“ä¸ºä¸­å›½äººï¼Œéœ€ç¬¦åˆåŒ—äº¬æ—¶é—´ä½œæ¯ä¹ æƒ¯
...
- reasoning (string): ç®€è¦è¯´æ˜Žä¸ºä»€ä¹ˆè¿™æ ·é…ç½®"""
        ...
        system_prompt = "ä½ æ˜¯ç¤¾äº¤åª’ä½“æ¨¡æ‹Ÿä¸“å®¶ã€‚è¿”å›žçº¯JSONæ ¼å¼ï¼Œæ—¶é—´é…ç½®éœ€ç¬¦åˆä¸­å›½äººä½œæ¯ä¹ æƒ¯ã€‚"
...
- **æ—¶é—´ç¬¦åˆä¸­å›½äººä½œæ¯**ï¼šå‡Œæ™¨0-5ç‚¹å‡ ä¹Žä¸æ´»åŠ¨ï¼Œæ™šé—´19-22ç‚¹æœ€æ´»è·ƒ
...
{{
    "agent_configs": [
        ...
        system_prompt = "ä½ æ˜¯ç¤¾äº¤åª’ä½“è¡Œä¸ºåˆ†æžä¸“å®¶ã€‚è¿”å›žçº¯JSONï¼Œé…ç½®éœ€ç¬¦åˆä¸­å›½äººä½œæ¯ä¹ æƒ¯ã€‚"
