    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? 'StartIn...' : 'StartGenerateResultReport' }} 
          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- GeneralRollback：UnknownTypeOr Has content But Not By AboveProcess -->
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // FromStep2Passed MaxRound Count
  minutesPerRound: {
    type: Number,
    default: 30 // DefaultPer Round30min
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const phase = ref(0) // 0: NotStart, 1: Running, 2: Completed
const isStarting = ref(false)
const isStopping = ref(false)
const startError = ref(null)
const runStatus = ref({})
const allActions = ref([]) // AllAction（IncrementAccumulate）
const actionIds = ref(new Set()) // Used ForDeduplicateActionIDCollection
const scrollContainer = ref(null)

// Computed
// By Time OrderShowAction（Newest at End，To Bottom）
const chronologicalActions = computed(() => {
  return allActions.value
})

// EachPlatformActionCount
...
// FormatFormatSimulationElapsed Time（By Round and Per RoundminNumber Calculation）
...
// TwitterPlatformSimulationElapsed Time
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// RedditPlatformSimulationElapsed Time
...
// ResetAllStatus（Used ForRestartStartSimulation）
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
...
const doStartSimulation = async () => {
  ...
  // FirstResetAllStatus，EnsureNot affectedtoLastSimulationInfluence Of
  resetAllState()
  
  isStarting.value = true
  startError.value = null
  addLog('CurrentlyStartDoublePlatformAndRowSimulation...')
  ...
  try {
    const params = {
      ...
      platform: 'parallel',
      force: true,  // Force RestartStart
      enable_graph_memory_update: true  // OpenStartStateGraphUpdate
    }
    
    if (props.maxRounds) {
      params.max_rounds = props.maxRounds
      addLog(`Set MaximumSimulationRound Count: ${props.maxRounds}`)
    }
    
    addLog('OpenedStartStateGraphUpdateMode')
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Cleaned OldSimulationLog，RestartStart Simulation')
      }
      addLog('✓ SimulationEngineStartSuccess')
...
const handleStopSimulation = async () => {
  ...
  addLog('CurrentlyStopSimulation...')
  
  try {
    const res = await stopSimulation({ simulation_id: props.simulationId })
    
    if (res.success) {
      addLog('✓ SimulationStopped')
      phase.value = 2
      stopPolling()
...
// Track EachPlatformLast Round Of，Used ForDetectChangeAndOutputLog
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // RespectivelyDetectEachPlatformRoundChangeAndOutputLog
      ...
      // DetectSimulationWhetherCompleted（Pass runner_status OrPlatformCompleteStatusJudge）
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Extra Check：IfBackend not ready yetUpdate runner_status，ButPlatformAlreadyReportComplete
      // PassDetect twitter_completed And reddit_completed StatusJudge
      const platformsCompleted = checkPlatformsCompleted(data)
      
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ DetecttoAllPlatformSimulationEnded')
        }
    ...
    console.warn('GetOperationRowStatusFailed:', err)
  }
}

// CheckAllEnabledPlatformWhetherCompleted
const checkPlatformsCompleted = (data) => {
  // IfNo anyPlatformData，Return false
  if (!data) return false
  
  // Check EachPlatformCompleteStatus
  const twitterCompleted = data.twitter_completed === true
  const redditCompleted = data.reddit_completed === true
  
  // IfAt least onePlatformCompleteDone，CheckWhetherAllEnabledPlatformAllCompleteDone
  // Pass actions_count JudgePlatformWhetherEnabled（If count > 0 Or running Once Was true）
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted
  
  // IfNo anyPlatformEnabled，Return false
  if (!twitterEnabled && !redditEnabled) return false
  
  // CheckAllEnabledPlatformWhetherAllCompleted
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // Use all_actions GetCompleteActionColumnTable
      const serverActions = res.data.all_actions || []
      
      // IncrementAddNewAction（Deduplicate）
      ...
      // NotAutoScroll，LetUserFreeViewTime Axis
      // NewActionWillBottomAppend
    }
  } catch (err) {
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Report GenerationRequest Sent，Please Wait...')
    return
  }
  
  isGeneratingReport.value = true
  addLog('CurrentlyStartReport Generation...')
...
onMounted(() => {
  addLog('Step3 SimulationOperationRowInitialize')
  if (props.simulationId) {
    doStartSimulation()
  }
})
