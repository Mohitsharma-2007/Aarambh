    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? 'GraphRAGLong-short Term MemoryReal-timeUpdateIn' : 'Real-timeUpdateIn...' }}
        </div>
        
        <!-- SimulationEndAfterHint -->
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Still Small AmountContentProcessIn，Suggest Manual LaterRefreshGraph</span>
      ...
      <div v-else class="graph-state">
        ...
        <p class="empty-text">WaitingOntologyGenerate...</p>
      </div>
    </div>

    <!-- BottomLegend (Bottom Left) -->
...
const showEdgeLabels = ref(true) // Default ShowEdgeLabel
const expandedSelfLoops = ref(new Set()) // ExpandSelf-loop Item Of
const showSimulationFinishedHint = ref(false) // SimulationEndAfterHint
const wasSimulating = ref(false) // Track beforeWhetherInSimulationIn
...
// Monitor isSimulating Change，DetectSimulationEnd
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // FromSimulationBecome non inSimulationStatus，ShowEndHint
    showSimulationFinishedHint.value = true
  }
  wasSimulating.value = newValue
}, { immediate: true })

// SwitchSelf-loopItemExpand/CollapseStatus
...
// CalculateEntityTypeUsed ForLegend
...
const renderGraph = () => {
  ...
  // ProcessEdgeData，CalculateSame pairNodeEdges betweenQuantityAnd index
  const edgePairCount = {}
  const selfLoopEdges = {} // Self-loops Grouped by Node
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // StatisticsPer PairNodeEdges betweenQuantity，CollectSelf-loopEdge
  tempEdges.forEach(e => {
    if (e.source_node_uuid === e.target_node_uuid) {
      // Self-loop - CollecttoIn Array
  ...
  // RecordCurrentProcesstoWhich For Node PairitemsEdge
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // AlreadyProcessSelf-loop Node Of
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // Self-loopEdge - Per Node OnlyAddOneitemsMerged Self-loops
      if (processedSelfLoopNodes.has(e.source_node_uuid)) {
        return // AlreadyProcessPass，Skip
      }
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // Store All Self-loop DetailsInformation
        }
      })
      return
    }
    ...
    // Determine edge directionWhetherConsistent with standard direction（SourceUUID < TargetUUID）
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // CalculateCurvature：MultiitemsSpread When Edge，SingleitemsEdge isStraight Line
    let curvature = 0
    if (totalCount > 1) {
      // Uniform Distribution Curvature，EnsureClearly distinguish
      // Curvature range based on edgesQuantityIncrease，More EdgesLarger curvature range
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // IfEdge direction opposite to standard，Flip Curvature
      // ThusEnsureAllEdges distributed in same reference frame，No overlap due to different directions
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Simulation - According toEdgeQuantityDynamicAdjustNodeSpacing
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Based on this pairNodeEdges betweenQuantityDynamicAdjustDistance
      // BasicDistance 150，Each AdditionalitemsEdge Increase 40
    ...
    // AddGravity to Center，Let independentNodeGroup clustertoCenterArea
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const loopRadius = 30
      // FromNodeStart from right，Loop Back
      const x1 = sx + 8  // Start Offset
      const y1 = sy - 4
      const x2 = sx + 8  // End Offset
      const y2 = sy + 4
      // UseArc drawingSelf-loop（sweep-flag=1 Clockwise）
    ...
    // CalculateCurveControl Point - According toEdgeQuantityAnd distance dynamicAdjust
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Perpendicular Offset，Calculate by Distance Ratio，EnsureCurveClearly visible
    // More Edges，Larger offset-to-distance ratio
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // Basic25%，Each AdditionalitemsEdge Increase5%
  ...
  // CalculateCurveMidpoint（Used ForLabelPosition）
  const getLinkMidpoint = (d) => {
    ...
    // DetectSelf-loop
    if (d.isSelfLoop) {
      // Self-loopLabelPosition：NodeRight Side
      return { x: sx + 70, y: sy }
    }
    ...
    // Quadratic BezierCurveMidpoint t=0.5
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    const pairTotal = d.pairTotal || 1
    ...
    // Quadratic BezierCurveFormula B(t) = (1-t)²P0 + 2(1-t)tP1 + t²P2, t=0.5
  ...
  // Link labels background (White background makes textClearer)
  ...
  // SaveReference for External Show/Hide
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Only RecordPosition，Do not restart simulation（DistinguishClickAndDrag）
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // DetectWhetherTrulyStartDrag（Move Beyond Threshold）
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // First TimeDetecttoTrulyDrag，Restart Simulation Only
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // OnlyTrulyDragOnly then gradually stop simulationStop
        if (d._isDragging) {
    ...
    .on('click', (event, d) => {
      ...
      // HighlightWith ThisNodeConnected edges
  ...
  simulation.on('tick', () => {
    ...
    // UpdateEdgeLabelPosition（NoRotate，HorizontalShowClearer）
    linkLabels.each(function(d) {
      const mid = getLinkMidpoint(d)
      d3.select(this)
        .attr('x', mid.x)
        .attr('y', mid.y)
        .attr('transform', '') // RemoveRotate，KeepHorizontal
    })
    
    // UpdateEdgeLabelBackground
  ...
  // Click Blank AreaCloseDetailsPanel
  svg.on('click', () => {
    selectedItem.value = null
    node.attr('stroke', '#fff').attr('stroke-width', 2.5)
    linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
