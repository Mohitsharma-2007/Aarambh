    ...
    <div class="main-split-layout">
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <!-- Next Step Button - InCompleteAfterShow -->
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (ContentGenerateComplete，But WholeChapterMay Not YetComplete) -->
                  ...
                  <!-- Section Complete (ChapterGenerateComplete) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- When Final Answer，ShowSpecialHint -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // Waiting for DOM UpdateAfter,Adjust Scroll Position to Keep Button at Same Position
...
const toggleSectionCollapse = (idx) => {
  // Only Completed Chapter OfCanCollapse
  if (!generatedSections.value[idx + 1]) return
...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // Lightbulb Icon - Represent insight
  },
  'panorama_search': {
    name: 'Panorama Search',
    color: 'blue',
    icon: 'globe' // Globe Icon - Represent panoramaSearch
  },
  'interview_agents': {
    name: 'Agent Interview',
    color: 'green',
    icon: 'users' // Users Icon - RepresentDialog
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // Lightning Icon - Represent fast
  },
  'get_graph_statistics': {
    name: 'Graph Stats',
    color: 'cyan',
    icon: 'chart' // Chart Icon - RepresentStatistics
  },
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // Database Icon - RepresentEntity
  }
}
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // ExtractStatisticsData - Match"RelatedPredicted facts: Xitems"Format
    const factMatch = text.match(/RelatedPredicted facts:\s*(\d+)/)
    ...
    // ExtractSub-question - Full Extraction，No Limit
    ...
    // ExtractKey facts - Full Extraction，No Limit
    ...
    // ExtractCore Entities - Full Extraction，ContainsSummaryAndRelatedFact count
    const entitySection = text.match(/### 【Core Entities】\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
      const entityText = entitySection[1]
      // By "- **" SplitEntity Block
    ...
    // ExtractRelationship Chain - Full Extraction，No Limit
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // ExtractInvolved Entities - Full Extraction，No Limit
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // ExtractInterview Count（As "5 / 9 PositionSimulationAgent"）
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // For Example: 1. **Alumni_345（index=1）**：As Wuhan UniversityAlumni...
        ...
        // Format2: - Selected Name（index X）：Reason
        // For Example: - Selected Parent_601（index 0）：As ParentGroup representative...
        if (!headerMatch) {
        ...
        // For Example: - **Parent_601（index 0）**：As ParentGroup representative...
        if (!headerMatch) {
        ...
        } else if (currentName && line.trim() && !line.match(/^Not Selected|^In Summary|^Final Selectionion/)) {
          // ReasonContinuation Row（Exclude Ending Summary）
          currentReason.push(line.trim())
        }
      }
      
      // Save Last Person Reason
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // ExtractTitle（As "Student"、"Education Practitioner" Etc）
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // If First QuestionHas Before"1."，NeedSpecialProcess
      ...
      // ExtractAnswer - SplitTwitterAndReddit
      ...
      if (answerMatch) {
        ...
        // Platform RollbackLogic（Compatible with OldFormat：Only OnePlatform MarkCase）
        if (!twitterMatch && redditMatch) {
          // Only Reddit Answer，Only WhenNon-placeholder TextCopy AsDefault Show
          ...
          // No SeparationPlatform Mark（Very Old Format），Whole AsAnswer
          interview.twitterAnswer = answerText
        }
      }
      
      // ExtractKey Quotes（Compatible with multiple quote typesFormat）
      const quotesMatch = block.match(/\*\*Key Quotes:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // Priority Match > "text" Format
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // Rollback：Match > "text" Or > \u201Ctext\u201D（Chinese Quotes）
        if (!quoteMatches) {
    ...
    const summaryMatch = text.match(/### InterviewSummaryWithCoreView\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/Findto\s*(\d+)\s*items/)
    if (countMatch) result.count = parseInt(countMatch[1])
    
    // ExtractRelated Facts - Full Extraction，No Limit
    ...
    // TryExtractEdgeInformation（IfHave）
    const edgesSection = text.match(/### RelatedEdge:\n([\s\S]*?)(?=\n###|$)/)
    if (edgesSection) {
    ...
    // TryExtractNodeInformation（IfHave）
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Latest Key Facts in Timeline Memory'),
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'None YetCurrent Key Memory'),
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'None YetCore Entities'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'None YetRelationship Chain')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `CurrentValid Memory (${props.result.activeFacts.length})`)
        ]),
        h('button', {
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'CurrentValid Memory'),
          ...
          ) : h('div', { class: 'empty-state' }, 'None YetCurrentValid Memory'),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
            ...
            (expandedHistorical.value ? props.result.historicalFacts : props.result.historicalFacts.slice(0, INITIAL_SHOW_COUNT)).map((fact, i) => 
              h('div', { class: 'fact-item historical', key: i }, [
                ...
                h('div', { class: 'fact-content' }, [
                  // TryExtractTimeInformation [time - time]
                  (() => {
          ...
          ) : h('div', { class: 'empty-state' }, 'None YetHistory Memory'),
        ...
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'None YetInvolved Entities'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // CheckWhetherForPlatformPlaceholder text
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '（No Reply on This Platform）' || t === '(No Reply on This Platform)' || t === '[No Reply]'
    }

    // TryByQuestionNumber splitAnswer
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Support Two Number Formats：
      // 1. "QuestionX：" Or "QuestionX:" （ChineseFormat，Backend New Format）
      // 2. "1. " Or "\n1. " （Number+Point，Old Format Compatible）
      let matches = []
      let match

      // PriorityTry "QuestionX：" Format
      const cnPattern = /(?:^|[\r\n]+)Question(\d+)[：:]\s*/g
      ...
      // IfNot matchedto，Rollbackto "Number." Format
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // IfNot FoundtoNumber or Only FindtoOne，ReturnWhole
      if (matches.length <= 1) {
    ...
    // GetCertainQuestionCorrespondingAnswer
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // SplitSuccessAnd index valid
      if (answers.length > 1 && qIdx < answers.length) {
        return answers[qIdx] || ''
      }

      // SplitFailed：First QuestionReturnCompleteAnswer，RestReturnEmpty
      return qIdx === 0 ? answer : ''
    }
    
    // Check certainQuestionWhetherHas DoublePlatformAnswer（FilterPlaceholder text）
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // TwoPlatformAll have realAnswer（Non-placeholder Text）AndContentDifferent
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // DoublePlatformSwitchButton（Only when has real doublePlatformAnswerTimeShow）
                  ...
                  // Expand/Collapse Button（Placeholder text notShow）
    ...
    return () => h('div', { class: 'quick-search-display' }, [
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'Not FoundtoRelatedResult'),
          props.result.facts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedFacts.value = !expandedFacts.value }
          }, expandedFacts.value ? `Collapse ▲` : `Expand All ${props.result.facts.length} items ▼`)
        ...
        activeTab.value === 'edges' && hasEdges.value && h('div', { class: 'edges-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'RelatedRelationship'),
        ...
        activeTab.value === 'nodes' && hasNodes.value && h('div', { class: 'nodes-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'RelatedNode'),
...
// CurrentActive Step（Used ForTopShow）
const activeStep = computed(() => {
  const steps = workflowSteps.value
  // FindtoCurrent active Step Of
  const active = steps.find(s => s.status === 'active')
  if (active) return active
  
  // IfNo active，ReturnLast done Step Of
...
const renderMarkdown = (content) => {
  ...
  // Remove Leading H2（## xxx），Chapter Title Already OutsideShow
  let processedContent = content.replace(/^##\s+.+\n+/, '')
  
  // ProcessCode Block
  let html = processedContent.replace(/```(\w*)\n([\s\S]*?)```/g, '<pre class="code-block"><code>$2</code></pre>')
  
  // ProcessRowInternal code
  ...
  // ProcessColumnTable - Support ChildColumnTable
  ...
  // Wrap unorderedColumnTable
  html = html.replace(/(<li class="md-li"[^>]*>.*?<\/li>\s*)+/g, '<ul class="md-ul">$&</ul>')
  // Wrap orderedColumnTable
  html = html.replace(/(<li class="md-oli"[^>]*>.*?<\/li>\s*)+/g, '<ol class="md-ol">$&</ol>')

  // CleanColumnBetween table itemsAllWhitespace
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // CleanColumnTableStartLabelAfterWhitespace
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // CleanColumnTableEndLabelWhitespace before
  ...
  // ProcessSwitchRow - EmptyRowBecomeParagraph separator，Single LineRowBecome <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // CleanEmpty paragraph
  html = html.replace(/<p class="md-p"><\/p>/g, '')
  html = html.replace(/<p class="md-p">(<h[2-5])/g, '$1')
  html = html.replace(/(<\/h[2-5]>)<\/p>/g, '$1')
  html = html.replace(/<p class="md-p">(<ul|<ol|<blockquote|<pre|<hr)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>|<\/pre>)<\/p>/g, '$1')
  // CleanBefore block elementAfter <br> Label
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // Clean <p><br> Case Following Block Element（Extra EmptyRowCause）
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // CleanConsecutive <br> Label
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // CleanParagraph after block elementStartLabelBefore <br>
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-continuous orderingColumnTable Number：When Single <ol> By ParagraphContentWhen Separated，Keep numbering incremental
...
const getLogLevelClass = (log) => {
  ...
  // INFO UseDefaultColor，NotMarkFor success
  return ''
}
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          // section_complete - ChapterGenerateComplete
          if (log.action === 'section_complete') {
            if (log.details?.content) {
              generatedSections.value[log.section_index] = log.details.content
              // AutoExpandJustGenerateChapter Of
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // EnsureClear loading Status
            emit('update-status', 'completed')
            stopPolling()
            // ScrollLogic unified in loopEndAfter nextTick InProcess
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // IfTaskCompleted，ScrolltoTop；Otherwise ScrolltoBottomFollow latestLog
            if (isComplete.value) {
...
// ExtractFinal AnswerContent - From LLM response InExtractChapterContent
const extractFinalContent = (response) => {
  if (!response) return null
  
  // TryExtract <final_answer> LabelInsideContent
  ...
  // TryFind Final Answer: FollowingContent（Support Multiple Formats）
  ...
  // TryFind Final Answer: FollowingContent
  const chineseFinalMatch = response.match(/Final Answer[:：]\s*\n*([\s\S]*)$/i)
  if (chineseFinalMatch) {
    return chineseFinalMatch[1].trim()
  }
  
  // IfBy ## Or # Or > Beginning，May Be Direct markdown Content
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // IfContentLonger And ContainsmarkdownFormat，TryRemoveReturn After Thinking
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remove Thought: BeginningThinking process
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* Dark Gray，Not FollowStatusChange */
  font-weight: 500;
}
