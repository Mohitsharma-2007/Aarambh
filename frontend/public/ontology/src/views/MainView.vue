...
const stepNames = ['å›¾è°±æž„å»º', 'çŽ¯å¢ƒæ­å»º', 'å¼€å§‹æ¨¡æ‹Ÿ', 'æŠ¥å‘Šç”Ÿæˆ', 'æ·±åº¦äº’åŠ¨']
...
const handleNextStep = (params = {}) => {
  if (currentStep.value < 5) {
    currentStep.value++
    addLog(`è¿›å…¥ Step ${currentStep.value}: ${stepNames[currentStep.value - 1]}`)
...
const handleGoBack = () => {
  if (currentStep.value > 1) {
    currentStep.value--
    addLog(`è¿”å›ž Step ${currentStep.value}: ${stepNames[currentStep.value - 1]}`)
  }
}
